...
class Conv2D:
    def __init__(self, num_filters, filter_size, input_shape, stride=1, padding=0):
        ...
        self.filter_size = filter_size  # assumed square
        self.stride = stride
    ...
    def init_weights(self):
        ...
        self.filters = np.random.uniform(-limit, limit, 
                        (self.num_filters, c, self.filter_size, self.filter_size))
    ...
    def forward(self, input):
        ...
        self.h_out = int((h - self.filter_size + 2 * self.padding) / self.stride) + 1
        self.w_out = int((w - self.filter_size + 2 * self.padding) / self.stride) + 1
